...
# model = SuperLightMobileNet(4).to(device)
# # 모델 로드 (미리 로드해 두기)
# # model = torch.load('pytorchToDjangoTest/model_30.pth',map_location=torch.device('cpu'))
# state_dict = torch.load('pytorchToDjangoTest/model_30_team2_cnn-2.pth', map_location=torch.device('cpu'))
...
#             class_labels = {0: 'cpu', 1: '그래픽 카드', 2: '파워', 3: '램'}
#
#             # 모델 예측
#             with torch.no_grad():
#                 outputs = model(image)
